        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture) {
            ...
            return targetType.Name switch {
                ...
                nameof(Visibility) => value switch {
                    ...
                    CebStatus status => (inverse && !(status == CebStatus.CompteApproche || status == CebStatus.CompteEstBon || status == CebStatus.Invalide)) ||
                                        (!inverse && (status == CebStatus.CompteApproche || status == CebStatus.CompteEstBon || status == CebStatus.Invalide))
