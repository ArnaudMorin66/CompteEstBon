...
namespace CompteEstBon {
    ...
    public partial class MainWindow {
        ...
        private void BtnSize_Click(object sender, RoutedEventArgs e) => WindowState =
            (WindowState == WindowState.Normal) ? WindowState.Maximized : WindowState.Normal;

        private void DisplaySystemMenu(object obj) {
            if(obj is not UIElement elt)
                return;
            var hWnd = new System.Windows.Interop.WindowInteropHelper(this).Handle;
            var hMenu = NativeMethods.GetSystemMenu(hWnd, false);
            var location = elt.PointToScreen(Mouse.GetPosition(elt));
            var cmd = NativeMethods.TrackPopupMenu(hMenu, 0x100, (int)location.X, (int)location.Y, 0, hWnd, nint.Zero);
            if(cmd > 0)
                NativeMethods.SendMessage(hWnd, 0x112, (nint)cmd, nint.Zero);
        }
        ...
        private void TxtSearch_LostFocus(object sender, RoutedEventArgs e) { txtSearch.SelectAll(); }

        private void TxtSearch_GotFocus(object sender, RoutedEventArgs e) {
            var textbox = sender as TextBox;
            var r = int.TryParse(textbox?.Text, out var v);
            if(r != false && v is >= 100 and <= 999)
                return;
            txtSearch.SelectAll();
            e.Handled = true;
        }
        ...
        private void Window_MouseDoubleClick(object sender, MouseButtonEventArgs e) => WindowState =
            (WindowState == WindowState.Normal) ? WindowState.Maximized : WindowState.Normal;
