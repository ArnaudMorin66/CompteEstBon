...
namespace CompteEstBon {
    public static class ExportData {
        public static async void ToWordAsync(this CebTirage tirage) {
            try {
        ...
        public static async void ToExcelAsync(this CebTirage tirage) {
            using var engine = new ExcelEngine();
            var excel = engine.Excel.Application;
            excel.DefaultVersion = ExcelVersion.Excel2016;

            var workBook = excel.Workbooks.Create();
            var ws = workBook.Worksheets[1];
            
            for (var i = 1; i <= 6; i++) {
               
                ws.Range[1, i + 1].Value = $"Plaque {i}";
                ws.Range[2, i + 1].Value = tirage.Plaques[i - 1].Text;
            }

            ws.Range[1, 8].Value = "Recherche";
            ws.Range[2, 8].Value = tirage.Search.ToString();
            var styletb = tirage.Status == CebStatus.CompteEstBon ? "TableStyleMedium7" : "TableStyleMedium3";
            var ls = ws.ListObjects.Create("TbSolutiond", ws.Range["B1"].)
