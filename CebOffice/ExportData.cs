...
namespace CompteEstBon {
    public static class ExportData {
        ...
        public static void ToExcel(this CebTirage tirage) {
            ...
            for (var i = 1; i <= 6; i++) {
                _r(ws.Cells[1, i + 1]).Value = $"Plaque {i}";
                _r(ws.Cells[2, i + 1]).Value = tirage.Plaques[i - 1];
            }


            _r(ws.Cells[1, 8]).Value = "Recherche";
            _r(ws.Cells[2, 8]).Value = tirage.Search;
